...
select eid,ename,department_name,salary from employee
 where salary>(select avg(salary) from employee as e where e.department_name=employee.department_name);
 select e.eid,e.ename,e.department_name,e.salary from employee as e
 join employee as emp
 on e.department_name=emp.department_name
 group by department_name
 having avg(salary)>=e.department_name;
 
 
 select ename,department_name,salary from employee
 where salary<=all(select salary from employee as e where e.department_name=employee.department_name);
